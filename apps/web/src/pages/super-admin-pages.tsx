...
export function SuperAdminHomePage() {
  return (
    <section>
      <h1 className="text-3xl font-black">Quản trị hệ thống</h1>
      <p className="mt-3">
...
export function SuperAdminUsersPage() {
  ...
  if (users.isPending) return <p>Đang tải người dùng…</p>;
  if (users.isError) return <p role="alert">Không thể tải người dùng.</p>;
  return (
    <section className="w-full">
      ...
      <input
        ...
        value={search}
      />
      <form className="mt-4 grid gap-2 rounded border p-3 sm:grid-cols-3" onSubmit={submit}>
      ...
      {users.data.items.length === 0 ? (
        <p className="mt-4">Không có kết quả.</p>
      ) : (
        <ul className="mt-4 space-y-2">
          {users.data.items.map((user) => (
            <li className="rounded border p-3" key={user.id}>
              <Link className="font-bold text-primary" to={`/super-admin/users/${user.id}`}>
                {user.username}
              </Link>{' '}
              — {user.role} — {user.isActive ? 'Hoạt động' : 'Đã tắt'}
            </li>
          ))}
        </ul>
      )}
...
export function SuperAdminUserPage() {
  const { id = '' } = useParams();
  const cache = useQueryClient();
  const user = useQuery({
  ...
  const change = useMutation({
    mutationFn: (body: unknown) => superAdminApi.update(id, body),
    onSuccess: () => {
  ...
  if (user.isPending) return <p>Đang tải…</p>;
  if (user.isError) return <p role="alert">Không tìm thấy người dùng.</p>;
  return (
    <section>
      <h1 className="text-3xl font-black">{user.data.username}</h1>
      <p>
        {user.data.fullName} · {user.data.email}
      </p>
      <p>Vai trò: {user.data.role}</p>
      <p>Entitlement: {user.data.adminEntitlement?.status ?? 'Không có'}</p>
      <select
        className="mt-3"
        defaultValue={user.data.role}
        onChange={(e) => {
          if (confirm('Đổi vai trò?'))
            change.mutate({ role: e.target.value, reason: 'Super-admin action' });
        }}
      >
        <option>USER</option>
        <option>ADMIN</option>
      ...
      <button
        className="ml-2 rounded bg-destructive px-3 py-2 text-white"
        onClick={() => {
          if (confirm('Vô hiệu hóa người dùng này?'))
            change.mutate({ isActive: false, reason: 'Super-admin action' });
        }}
        type="button"
      >
        Vô hiệu hóa
      ...
      <button
        className="ml-2"
        onClick={() => {
          if (confirm('Đình chỉ quyền quản trị?'))
            access.mutate({ action: 'SUSPEND', reason: 'Super-admin action' });
        }}
        type="button"
      >
        Đình chỉ
      </button>
      {change.isError || access.isError ? (
