...
export function TournamentsPage() {
  const query = useQuery({
    ...
    queryFn: publicViewApi.tournaments,
  });
...
export function TournamentPage() {
  ...
  const query = useQuery({
    ...
    queryFn: () => publicViewApi.tournament(id),
    enabled: Boolean(id),
  });
  if (query.isPending) return <Notice>Đang tải giải đấu…</Notice>;
  ...
  return (
    <section className="mx-auto w-full max-w-3xl">
      ...
      <ul className="mt-3 space-y-2">
        {query.data.tournament.matches.map((match) => (
          <li key={match.id}>
...
export function MatchPage() {
  ...
  const query = useQuery({
    ...
    queryFn: () => publicViewApi.match(id),
    enabled: Boolean(id),
  });
...
export function AccountPage() {
  ...
  return (
    <Notice>
      <h1 className="text-2xl font-black">Tài khoản</h1>
      <p className="mt-3">{query.data.user.fullName ?? query.data.user.username}</p>
      <p className="text-sm text-muted-foreground">{query.data.user.username}</p>
    </Notice>
  );
}
