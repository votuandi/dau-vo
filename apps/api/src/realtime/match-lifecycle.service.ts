...
export interface MatchExitTransition {
  matchId: string;
  matchPublicId: string;
  tournamentId: string;
  payload: MatchExitPayload;
  releasedOfficialIds: string[];
}
  ...
  async exitMatch(input: {
    ...
    const result = await this.prisma.$transaction(
      async (transaction) => {
        ...
        const retainsV2Phase =
          input.mode === MatchExitMode.SUSPEND_KEEP_V2_PHASE;
      ...
      async (transaction) => {
        ...
        const selectedRounds = entireMatch
          ...
          : match.status === MatchStatus.REGULATION_APPEAL
            ...
            : overtimePhase
              ? activeRounds
                  .filter((round) => round.stage === RoundStage.OVERTIME)
                  .slice(0, 1)
        ...
        const roundNumbers = [
          ...new Set(selectedRounds.map((round) => round.roundNumber)),
        ] as Array<1 | 2>;
        ...
        const nextStatus =
          ...
          selectedRounds.some(
            ...
            ? MatchStatus.WAITING
            : selectedRounds.some(
                  (round) => round.stage === RoundStage.OVERTIME,
                )
              ? MatchStatus.OVERTIME_READY
              : MatchStatus.BREAK;
        const audit = await transaction.auditLog.create({
        ...
        const operation = await transaction.matchResultOperation.create({
          data: {
            ...
            matchId: input.matchId,
            previousCurrentRound: match.currentRound,
            previousLifecycle: this.lifecycleForPhase(match.status),
            previousFinishedAt: match.finishedAt,
            previousStartedAt: match.startedAt,
            previousStatus: match.status,
            resultingCurrentRound,
        ...
        await transaction.matchResultDecision.updateMany({
          ...
          where: {
            invalidatedAt: null,
            matchId: input.matchId,
            sourceOvertimeRoundId: { in: roundIds },
          },
        });
        await transaction.scoringWindow.updateMany({
          data: {
      ...
      async (transaction) => {
        ...
        const activitySinceCancellation = await Promise.all([
          ...
          transaction.matchAppeal.count({
            where: {
              completedAt: { gt: operation.createdAt },
              invalidatedAt: null,
              matchId: input.matchId,
            },
          }),
          transaction.matchResultDecision.count({
            where: {
              selectedAt: { gt: operation.createdAt },
              invalidatedAt: null,
              matchId: input.matchId,
            },
          }),
          transaction.matchOutcome.count({
            where: {
              publishedAt: { gt: operation.createdAt },
              matchId: input.matchId,
            },
          }),
