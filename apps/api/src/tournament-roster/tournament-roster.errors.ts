export const ROSTER_TOURNAMENT_ARCHIVED = {
  code: 'TOURNAMENT_ARCHIVED',
  message:
    'Tournament roster cannot be changed while the tournament is archived',
};
export const ORGANIZATION_NOT_FOUND = {
  code: 'ORGANIZATION_NOT_FOUND',
  message: 'Participating organization not found',
};
export const WEIGHT_CLASS_NOT_FOUND = {
  code: 'WEIGHT_CLASS_NOT_FOUND',
  message: 'Weight class not found',
};
export const ORGANIZATION_NAME_EXISTS = {
  code: 'ORGANIZATION_NAME_ALREADY_EXISTS',
  message: 'A participating organization with this name already exists',
};
export const WEIGHT_CLASS_NAME_EXISTS = {
  code: 'WEIGHT_CLASS_NAME_ALREADY_EXISTS',
  message: 'A weight class with this name already exists',
};
export const ORGANIZATION_UPDATE_EMPTY = {
  code: 'ORGANIZATION_UPDATE_EMPTY',
  message: 'Provide at least one organization property to update',
};
export const WEIGHT_CLASS_UPDATE_EMPTY = {
  code: 'WEIGHT_CLASS_UPDATE_EMPTY',
  message: 'Provide at least one weight class property to update',
};
export const WEIGHT_CLASS_IN_USE = {
  code: 'WEIGHT_CLASS_IN_USE',
  message: 'Weight class is referenced by an active athlete or match',
};
export const ATHLETE_NOT_FOUND = {
  code: 'ATHLETE_NOT_FOUND',
  message: 'Tournament athlete not found',
};
export const ROSTER_ASSIGNMENT_NOT_FOUND = {
  code: 'ROSTER_ASSIGNMENT_NOT_FOUND',
  message: 'Roster assignment not found',
};
export const ATHLETE_UPDATE_EMPTY = {
  code: 'ATHLETE_UPDATE_EMPTY',
  message: 'Provide at least one athlete property to update',
};
export const ATHLETE_IN_ACTIVE_BRACKET = {
  code: 'ATHLETE_IN_ACTIVE_BRACKET',
  message:
    'Athlete eligibility cannot be changed while a current bracket exists. Cancel or reset the bracket first.',
};
export const INVALID_BIRTH_YEAR = {
  code: 'INVALID_BIRTH_YEAR',
  message: 'birthYear must be between 1900 and the current UTC year',
};
