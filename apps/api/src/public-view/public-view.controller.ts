...
export class PublicViewController {
  ...
  async match(@Param('id', new ParseUUIDPipe()) id: string) {
    const match = await this.prisma.match.findFirst({ where: { id, tournament: { status: { in: visibleStatuses }, softDeletedAt: null } }, select: { id: true, publicId: true, status: true, currentRound: true, startedAt: true, finishedAt: true, athletes: { select: { name: true, organization: true, color: true } }, tournament: { select: { id: true, name: true, status: true } } } });
